...
(define (solve-help index-now max-index resoult-list)
  (if (> index-now max-index)
    ...
    (if (check-A (bignum-to-A-base-n order index-now))
      (begin (if (= (remainder index-now 100) 0) (begin (display "now,index=") (display index-now) (display "") (newline)) ) (solve-help (+ index-now 1) max-index (cons index-now resoult-list)))
